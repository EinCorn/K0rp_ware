...
for (let y = 0; y < height; y += 1) {
  ...
  for (let x = 0; x < width; x += 1) {
    ...
    const spoke = Math.abs(Math.sin(angle * 3))
    const ring = distance > 18 && distance < 52 && spoke > 0.78
    const core = distance < 22
    const alpha = ring || core ? 255 : 0
    const shade = core ? 226 : 172

    row.push(shade, Math.max(shade - 8, 0), Math.max(shade - 24, 0), alpha)
  }
